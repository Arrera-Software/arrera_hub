...
hub_gui::hub_gui(QWidget *parent)
    ...
    connection_arrera("arrera_hub",this)
{
    ui->setupUi(this);
    ...
    // Dans le constructeur de ton Store
    connect(&connection_arrera, &CArreraClient::messageReceived, this, [this](const QString &message){
        if (message.trimmed() == "stop") {
            this->close();
        }
    });

    connection_arrera.connectToServeur("ws://localhost:2026");
...
void hub_gui::on_application_installed(bool succes)
{
    ...
    page_load = false;
    connection_arrera.sendMessage("update_app");
